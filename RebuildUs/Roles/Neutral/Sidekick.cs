...
public class Sidekick : RoleBase<Sidekick>
{
    ...
    public override void FixedUpdate()
    {
        ...
        if (local != null)
        {
            ...
            if (Jackal.Exists)
            {
                ...
                for (var i = 0; i < jkPlayers.Count; i++) untargetablePlayers.Add(jkPlayers[i]);
            }
    ...
    public static void MakeButtons(HudManager hm)
    {
        ...
        SidekickSabotageLightsButton = new(() => { MapUtilities.CachedShipStatus.RpcUpdateSystem(SystemTypes.Sabotage, (byte)SystemTypes.Electrical); }, () => { return PlayerControl.LocalPlayer.IsRole(RoleType.Sidekick) && CanSabotageLights && PlayerControl.LocalPlayer.IsAlive(); }, () =>
        {
        ...
        }, () => { SidekickSabotageLightsButton.Timer = Helpers.SabotageTimer() + 5f; }, AssetLoader.LightsOutButton, ButtonPosition.Layout, hm, hm.AbilityButton, AbilitySlot.CommonAbilitySecondary, false, FastDestroyableSingleton<TranslationController>.Instance.GetString(StringNames.FixLights));
    }
