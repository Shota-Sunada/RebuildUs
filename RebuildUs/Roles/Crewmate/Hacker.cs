...
internal class Hacker : MultiRoleBase<Hacker>
{
    ...
    {
        if (!PlayerControl.LocalPlayer.IsRole(RoleType.Hacker) || !PlayerControl.LocalPlayer.IsAlive()) return;
        (int playerCompleted, _) = TasksHandler.TaskInfo(Player.Data);
        if (playerCompleted != _rechargedTasks) return;
        _rechargedTasks += RechargeTasksNumber;
        if (ToolsNumber > _chargesVitals) _chargesVitals++;
        if (ToolsNumber > _chargesAdminTable) _chargesAdminTable++;
    }
    ...
    internal static void MakeButtons(HudManager hm)
    {
        ...
        HackerAdminTableButton = new(() =>
                                     ...
                                     {
                                         if (_hackerAdminTableChargesText == null && _hackerVitalsChargesText == null) return Local._chargesAdminTable > 0 && MapSettings.CanUseAdmin;
        ...
        HackerVitalsButton = new(
                                 ...
                                 () => PlayerControl.LocalPlayer.IsRole(RoleType.Hacker) && MapSettings.CouldUseVitals && PlayerControl.LocalPlayer.IsAlive() && Helpers.GetOption(ByteOptionNames.MapId) != 0 && Helpers.GetOption(ByteOptionNames.MapId) != 3, () =>
                                 {
                                 ...
                                 FastDestroyableSingleton<HudManager>.Instance.UseButton.fastUseSettings[ImageNames.VitalsButton].Image, ButtonPosition.Layout,
                                 hm,
                                 hm.UseButton,
                                 AbilitySlot.CommonAbilitySecondary, true, 0f,
                                 () =>
                                 {
                                     HackerVitalsButton.Timer = HackerVitalsButton.MaxTimer;
                                     if (!HackerAdminTableButton.IsEffectActive) PlayerControl.LocalPlayer.moveable = true;
                                     if (!Minigame.Instance) return;
                                     if (Helpers.IsMiraHq) Local._doorLog.ForceClose();
                                     else Local._vitals.ForceClose();
                                 },
                                 false,
