...
internal partial class CustomOption
{
    ...
    {
        ControllerManager.Instance.OpenOverlayMenu(__instance.name,
    ...
    internal static void SettingMenuStart(GameSettingMenu __instance)
    {
        ...
        whatIsThis.transform.localPosition = new(whatIsThis.transform.localPosition.x - 0.4f,
            whatIsThis.transform.localPosition.y + 0.9f,
            whatIsThis.transform.localPosition.z);
        whatIsThis.transform.localScale *= Vector2.one * 0.9f;

        // Game Settings
        gameSettingsButton.transform.localPosition = new(gameSettingsButton.transform.localPosition.x - 0.2f,
            gameSettingsButton.transform.localPosition.y + 1.65f,
            gameSettingsButton.transform.localPosition.z);
        ...
        _impostorButton = CreateSettingButton(__instance,
            "RUImpostorSettingsButton",
            Tr.Get(TrKey.ImpostorSettingsButton),
            OptionPage.ImpostorSettings);
        _impostorTab = CreateSettingTab(__instance, "RUGeneralImpostorTab", CustomOptionType.Impostor);
        _crewmateButton = CreateSettingButton(__instance,
            "RUCrewmateSettingsButton",
            Tr.Get(TrKey.CrewmateSettingsButton),
            OptionPage.CrewmateSettings);
        ...
        _modifierButton = CreateSettingButton(__instance,
            "RUModifierSettingsButton",
            Tr.Get(TrKey.ModifierSettingsButton),
            OptionPage.ModifierSettings);
        ...
        foreach (CustomOption option in options)
        {
            if (option.IsHeader)
            {
                ...
                num -= 0.63f;
                     ...
                     && (option.Parent.GetSelectionIndex() == 0 && !option.HideIfParentEnabled
