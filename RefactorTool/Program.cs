...
internal class Program
{
    private static async Task Main(string[] args)
    {
        ...
        foreach (ProjectId projectId in solution.ProjectIds)
        {
            ...
            if (project == null || project.Name == "RefactorTool")
            {
                continue;
            }
            ...
            List<ISymbol> sortedSymbols =
            [
                .. symbolsToRename
                    .OrderBy(s => s.Kind switch
                    {
                        SymbolKind.Local => 0,
            ...
            foreach (ISymbol symbol in sortedSymbols)
            {
                ...
                try
                {
                    ISymbol currentSymbol = await SymbolFinder.FindSourceDefinitionAsync(symbol, newSolution);
    ...
    private static bool ShouldRename(ISymbol symbol)
    {
        ...
        bool isValidKind = symbol.Kind == SymbolKind.Local || symbol.Kind == SymbolKind.Field || symbol.Kind == SymbolKind.Parameter || symbol.Kind == SymbolKind.Method || symbol.Kind == SymbolKind.Property;
        if (!isValidKind)
        {
            return false;
        }
        ...
        // Exclude overrides
        if (symbol is IMethodSymbol method && method.IsOverride)
        {
            return false;
        }
        if (symbol is IPropertySymbol prop && prop.IsOverride)
        {
            return false;
        }
    ...
    private static async Task<Document> CleanupDocumentAsync(Document document)
    {
        SyntaxNode? root = await document.GetSyntaxRootAsync();
