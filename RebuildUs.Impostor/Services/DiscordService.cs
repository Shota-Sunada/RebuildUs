...
using DiscordConfig = RebuildUs.Impostor.Models.DiscordConfig;
...
public sealed class DiscordService : IDiscordService
{
    ...
    public DiscordService(ILogger<DiscordService> logger, IOptions<DiscordConfig> config, IServiceProvider serviceProvider)
    {
        _logger = logger;
        ...
        // Add worker clients
        foreach (var _ in _config.WorkerTokens) _clients.Add(CreateClient(false));
    }
    ...
    public async Task StartAsync()
    {
        ...
        for (var i = 0; i < _config.WorkerTokens.Count; i++)
        {
    ...
    public async Task StopAsync()
    {
        ...
        foreach (var client in _clients) await client.StopAsync();
    }
    ...
    public async Task UpdateStatusAsync(string roomCode, int playerCount, int maxPlayers, string mapName, string gameState, string playerLinkStatus)
    {
        ...
        if (_isAutoTrack && string.IsNullOrEmpty(_targetRoomCode)) _targetRoomCode = roomCode.ToUpper();
        ...
        var embed = new EmbedBuilder().WithTitle("Among Us 部屋状況").WithColor(Color.Blue).AddField("部屋コード", $"`{roomCode}`", true).AddField("人数", $"{playerCount} / {maxPlayers}", true).AddField("マップ", mapName, true).AddField("状況", gameState, true).AddField("連携状況", playerLinkStatus).WithFooter("RebuildUs Status Update").WithCurrentTimestamp().Build();
    ...
    private DiscordSocketClient CreateClient(bool isMain)
    {
        ...
        if (isMain) intents |= GatewayIntents.GuildMessages | GatewayIntents.MessageContent;

        var client = new DiscordSocketClient(new() { GatewayIntents = intents });
        client.Log += LogAsync;
    ...
    private async Task MessageReceivedAsync(SocketMessage message)
    {
        ...
        {
            var embed = new EmbedBuilder().WithTitle("RebuildUs Bot ヘルプ").WithDescription("以下のコマンドが利用可能です。").AddField("!rebuild help", "このヘルプを表示します。").AddField("!rebuild set-channel", "現在のチャンネルをステータス・通知送信先に設定します。").AddField("!rebuild notify off/on", "ゲームイベント通知の無効化/有効化を切り替えます。").AddField("!rebuild status <ROOMCODE>", "特定の部屋の状況を表示します。").AddField("!rebuild status auto", "最新の部屋を自動で追跡します。").WithColor(Color.Green).Build();
    ...
    {
        var builder = new ComponentBuilder().WithButton("メッセージを削除", "delete_message", ButtonStyle.Secondary);
